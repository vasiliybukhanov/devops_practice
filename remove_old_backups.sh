...
get_file_creation_date_from_name() {
	echo "$1" | awk -F '.' '{start = index($1, "-"); print substr($1, start + 1)}'
}
...
remove_file_if_older_than_2_days() {
	...
	if [[ file_age_in_seconds -gt two_days_ago_in_seconds ]]; then
		return
	fi
	...
	if [[ -z $path_to_file_on_remote ]]; then
		return
	fi
	...
	if [[ $local_file_sum == "$remote_file_sum" ]]; then
		rm -f "$backup_file"
	fi
}
...
for element in "$backups_location_dir"/*; do
	if [[ -d $element ]]; then
		for backup_file in "$element"/*; do
